...
export const MultipleFiles = (prop: IMultipleFilesProps) => {
  ...
  const submitFiles = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (prop.uploadFiles) {
      await prop.uploadFiles(prop.selectedFiles);
    }
      ...
      >
        ...
        <div className="form-group text-center">
          <i
            ...
            onClick={() =>
              document
                .getElementById(prop.fileInputId ?? "extra_files")
                ?.click()
            }
          ...
          <br />
          <span className="text-center d-4 my-3 font-weight-bold">
            [{prop.fileLabelText ?? "add files"}
          </span>
        </div>
